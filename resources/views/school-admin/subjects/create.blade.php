@extends('layouts.app')
@section('title', __('schooladmin.add_subject'))
@section('page-title', __('schooladmin.add_subject'))
@section('page-subtitle', __('schooladmin.add_subject_subtitle'))

@section('content')
<div class="max-w-2xl mx-auto animate-fade-up">
    <form method="POST" action="{{ route('school-admin.subjects.store') }}" class="space-y-6"
          x-data="{ loading: false, icon: '{{ old('icon', '📖') }}', color: '{{ old('color', '#2196F3') }}' }"
          @submit="loading = true">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required class="input"
                           placeholder="{{ __('schooladmin.subject_name_placeholder') }}">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code') }}" class="input" placeholder="MATH-101">
                </div>
                <div>
                    <label class="label">{{ __('schooladmin.weekly_hours') }} *</label>
                    <input type="number" name="weekly_hours" value="{{ old('weekly_hours', 4) }}" required min="1" max="20" class="input">
                </div>
                <div>
                    <label class="label">{{ __('schooladmin.display_order') }}</label>
                    <input type="number" name="order" value="{{ old('order', 0) }}" min="0" class="input">
                </div>
                <div class="md:col-span-2">
                    <label class="label">{{ __('schooladmin.subject_description') }}</label>
                    <textarea name="description" rows="3" class="input resize-none"
                              placeholder="{{ __('schooladmin.subject_description_placeholder') }}">{{ old('description') }}</textarea>
                </div>
            </div>

            <label class="flex items-center gap-2.5 cursor-pointer select-none pt-2 border-t border-bd">
                <input type="checkbox" name="is_active" value="1" checked class="w-4 h-4 rounded-lg accent-brand-500">

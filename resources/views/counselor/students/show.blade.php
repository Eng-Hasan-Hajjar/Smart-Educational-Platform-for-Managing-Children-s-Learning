...
@section('page-subtitle', __('counselor.student_profile'))

@section('content')
<div class="space-y-6">

    {{-- ══════════ Back + Actions ══════════ --}}
    <div class="flex items-center gap-3 flex-wrap animate-fade-up">
        <a href="{{ route('counselor.students.index') }}" class="btn-outline !py-2 !px-3 text-xs">
            <svg class="w-4 h-4 flip-rtl" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
            </svg>
            {{ __('app.back') }}
        </a>
        <a href="{{ route('counselor.reports.create') }}?student_id={{ $student->id }}" class="btn-primary !py-2 text-xs">
            📝 {{ __('counselor.write_report') }}
        </a>
        @if($student->parents->count())
        <a href="{{ route('messages.compose') }}?recipient_id={{ $student->parents->first()->id }}" class="btn-outline !py-2 text-xs">
            ✉️ {{ __('counselor.contact_parent') }}
        </a>
        @endif
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 stagger animate-fade-up" style="animation-delay:.06s">
        @foreach([
            ['label'=>__('counselor.lessons_completed'), 'value'=>$lessonProgress->where('is_completed',true)->count(), 'icon'=>'📚','ring'=>'brand'],
            ['label'=>__('counselor.avg_score'),          'value'=>$avgScore.'%',                                       'icon'=>'📊','ring'=>($avgScore>=60?'success':'danger')],
            ['label'=>__('counselor.attendance_rate'),    'value'=>$attendanceRate.'%',                                 'icon'=>'✅','ring'=>($attendanceRate>=75?'success':'danger')],
            ['label'=>__('counselor.assignments_done'),   'value'=>$submissions->count(),                               'icon'=>'📋','ring'=>'warning'],
        ] as $s)
        <div class="card">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="card animate-fade-up" style="animation-delay:.08s">
            ...
            <div class="text-center py-6 animate-fade">
                ...
                <p class="text-muted text-sm mt-2">{{ __('counselor.no_lesson_progress') }}</p>
            </div>
            @endforelse
        </div>
        ...
        <div class="card animate-fade-up" style="animation-delay:.10s">
            ...
            <div class="text-center py-6 animate-fade">
                <span class="text-3xl animate-float inline-block">📝</span>
                <p class="text-muted text-sm mt-2">{{ __('counselor.no_quiz_results') }}</p>
            </div>
            @endforelse
        </div>
        ...
        <div class="card animate-fade-up" style="animation-delay:.12s">
            <h3 class="font-bold text-main flex items-center gap-2 mb-4">
                ...
                {{ __('counselor.assignment_history') }}
            </h3>
            ...
            <div class="text-center py-6 animate-fade">
                <span class="text-3xl animate-float inline-block">📋</span>
                <p class="text-muted text-sm mt-2">{{ __('counselor.no_submissions') }}</p>
            </div>
            @endforelse
        </div>
        ...
        <div class="card animate-fade-up" style="animation-delay:.14s">
            <h3 class="font-bold text-main flex items-center gap-2 mb-4">
                ...
                {{ __('counselor.attendance_last_30') }}
            </h3>
            <div class="flex flex-wrap gap-1.5">
                ...
                @empty
                <p class="text-muted text-sm">{{ __('counselor.no_attendance_recorded') }}</p>
                @endforelse
            </div>
            @if($attendances->count())
            <div class="mt-3 pt-3 border-t border-bd">
                <div class="flex items-center justify-between mb-1.5 text-xs">
                    <span class="text-muted">{{ __('counselor.attendance_rate') }}</span>
    ...
    {{-- ══════════ Previous Reports ══════════ --}}
    <div class="card animate-fade-up" style="animation-delay:.16s">
        <h3 class="font-bold text-main flex items-center gap-2 mb-4">
            <span class="w-9 h-9 rounded-xl bg-accent-50 text-accent-600 flex items-center justify-center text-base">📜</span>
            {{ __('counselor.previous_reports') }}
        </h3>
        @forelse($reports as $report)
        <a href="{{ route('counselor.reports.show', $report) }}"
           class="flex items-center gap-3 p-3 rounded-2xl border border-bd hover:bg-hover transition mb-2 group">
            <div class="w-10 h-10 rounded-xl bg-brand-50 text-brand-600 flex items-center justify-center text-xl flex-shrink-0">📋</div>
            <div class="flex-1 min-w-0">
                <p class="text-sm font-bold text-main group-hover:text-brand-500 transition truncate">{{ $report->type_label }}</p>
                <p class="text-xs text-muted">{{ $report->semester->name }} · {{ $report->created_at->format('d/m/Y') }}</p>
            </div>
            <span class="badge-{{ $report->is_sent_to_parent ? 'green' : 'gray' }} flex-shrink-0">
        ...
        <div class="text-center py-6 animate-fade">
            <span class="text-3xl animate-float inline-block">📜</span>
            <p class="text-muted text-sm mt-2">{{ __('counselor.no_previous_reports') }}</p>
        </div>
        @endforelse
    </div>
</div>

<div x-data="{ open: false }" class="relative">
    <button @click="open = !open" class="theme-toggle !w-auto !px-3 gap-1.5 text-xs font-bold">
        <span class="text-base leading-none">{{ app()->getLocale() === 'ar' ? '🇸🇾' : '🇬🇧' }}</span>
    ...
    <div x-show="open" x-cloak @click.outside="open=false"
         ...
         class="dropdown-panel absolute {{ app()->getLocale() === 'ar' ? 'left-0' : 'right-0' }} mt-2 w-44 z-50 overflow-hidden origin-top">

        <a href="{{ route('language.switch', 'ar') }}"
           class="flex items-center gap-3 px-4 py-3 text-sm hover:bg-hover transition
                  {{ app()->getLocale() === 'ar' ? 'text-brand-600 font-bold bg-brand-50' : 'text-main' }}">
        ...
        <a href="{{ route('language.switch', 'en') }}"
           class="flex items-center gap-3 px-4 py-3 text-sm hover:bg-hover transition border-t border-bd
                  {{ app()->getLocale() === 'en' ? 'text-brand-600 font-bold bg-brand-50' : 'text-main' }}">
            <span class="text-lg">🇬🇧</span>

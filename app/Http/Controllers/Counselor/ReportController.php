...
class ReportController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $semesters = Semester::whereHas('academicYear', fn($q) => $q->where('school_id', $school->id))
            ->orderByDesc('id')
            ->get();
        ...
        if ($request->student_id) {
            ...
            $subjectsData = StudentAnalytic::where('student_id', $request->student_id)
                ->with('subject')
                ->get()
                ->map(fn($a) => [
        ...
        return view('counselor.reports.create', compact(
            'students', 'semesters', 'selectedStudent', 'subjectsData'
        ));
    }

    public function store(Request $request)
    {
        // إعادة إرسال تقرير موجود لولي الأمر
        if ($request->has('report_id')) {
            ...
            return redirect()->route('counselor.reports.show', $report)
                ->with('success', __('counselor.report_sent_success'));
        }

        $data = $request->validate([
            'student_id'        => 'required|exists:users,id',
            'semester_id'       => 'required|exists:semesters,id',
            'type'              => 'required|in:periodic,final,behavioral,custom',
            'counselor_notes'   => 'required|string',
            'recommendations'   => 'nullable|string',
            'send_to_parent'    => 'boolean',
        ]);

        $subjectsData = StudentAnalytic::where('student_id', $data['student_id'])
            ->with('subject')
            ->get()
            ->map(fn($a) => [
                'subject' => $a->subject->name ?? '—',
        ...
        return redirect()->route('counselor.reports.show', $report)
            ->with('success', __('counselor.report_created_success'));
    }
